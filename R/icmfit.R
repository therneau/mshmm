...
#  multi-state hazard model.  No hidden states
icmfit <- function(Y, X, id, cmap, qmat, beta, coef, exact, death, control) {
    # The maximization routine will icloglik routine, which will in turn
    #  call either the icm1 (no deriviative) or icm2 (derivatives) function
    #  in parallel for each id to return the likelihood for each.
    # So we don't have to pass every argument down the chain, the main ones
    #  are set here, before defining icm1 and icm2. They will be inherited
    #  via lexical scoping.  I don't know of using lexical scope is faster or
    #  slower than argument passing, it just makes it easier to write the
    #  calls.
    Y <- Y
    X <- X
    beta <- beta
    cmap <- cmap
    from.state <- row(qmat)[qmat>0]
    to.state <- col(qmat)[qmat>0]
    # the column names of cmap will be paste(from.state, to.state, sep=':')
    eps <- control$eps
    nstate <- nrow(qmat)
    qmap <- which(qmat >0) # the element of qmat corresponding to each
                           #  linear predictor column of eta
    ...
    icm1 <- function(who) {
        ...
        # We don't need a transition matrix for the last row for each subject.
        # The call to upper uses the Ward approx (nterm=0) rather than
        #  the Higham09.  The former seems to better match my pade routine.
        r2 <- length(rows)  # will always be at least 2 rows for a subject
        ...
        for (j in seq_along(rows)[-1]) {
            # multiply by the transition matrix from time j-1 to j (present)
            alpha <- alpha %*% Pmat[,, j-1]

            k <- state[j]  # the newly realized state
            if (k %in% exact) {
                # exact event time (death)
                rmat[qmap] <- exp(eta[j-1,]) # hazard just before this point
                temp <- sum(alpha * rmat[, state[j])
                alpha[-k] <- 0
                alpha[k]  <- temp
                 if (debug > 2) cat("A2: j=", j, "alpha=", alpha, "\n")
            }
            else if (k>0)  # observed state, not exact
                alpha[-k] <- 0  # prob(somewhere else) =0
            else {
                # censored, we know they are not dead
                alpha[death] <- 0
            }
  ...
  cmap.b1 <- makeindex(cmap[,b1, drop=FALSE])
    ...
    psetup <- function(rmat, qmap) {
        ...
        for (i in 1:n.eta) {
            temp2 <- temp
            exp.eta <- rmat[qmap[i]]  # elements of rmat are exp(eta)
            temp2[qmap[i]] <-  exp.eta
    ...
    # repeat the icm1 funciton, but retain derivatives
    icm2 <- function(who) {
        rows <- which(id ==uid[who])  # the subjects of interest
        eta <- X[rows,] %*% beta
        P.d  <- matrix(0., nparm, nstate)
