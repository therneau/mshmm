# These two function are called by lappy, each computes the loglik for a single
#  subject: icmlog1 does not compute derivatives, icmlog2 does

icmlog1 <- function(alpha, beta, X, Y, rindex, death, eps= 1e-8, debug=0) {
    # alpha = probability vector, will have 1 in the starting state
    # beta = matrix of coefficients
    # X, Y = data for this subject
    # rindex: used by the upper routine
    nstate <- lenth(alpha)
    ytime <- diff(Y[,1])  # the time increment
    eta <- X%*% beta
    ...
    myexp <- .Call("upper", nstate, eta[-r2,,drop=FALSE], 
                       ytime[-r2], rindex, 1e-7, 0)
    ucount <- c(r2-1, myexp$ties)
    Pmat <- array(myexp$P, dim=c(nstate, nstate, r2-1))
    ...
    for (j in 1:(r2-1) {
        alpha <- alpha * Pmat[,,i]
        status <- Y[i,2]
        if (status %in% death) { # an exact endpoint
            temp <- alpha* exp(eta[j,
        if (otype[j] == 2) {
