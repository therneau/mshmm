...
hmmfit <- function(ytime, ystat, x, id, otype, qmatrix, cmap, rfun, 
    ...
    rval <- list(loglik= flog, beta=beta)
    ...
    #    hmmdb: for debugging, pass back everything, (use mfun=hmmdb)
    #  



hmm1, hmm2, hmmboth, hmmderiv are pasted
    # in right here, before the final return(rval) and closing paren of the
    # hmmfit function, for the sake of lexical scoping.  The variables passed
    # in to the hmmfit function will be available to them, without having
    # to be passed through the maximizer functions.
    ...
    #    cat("rlist done\n"); browser()
        ...
        if (length(rows) > 1) {  # but add a failsafe
            ...
            if (debug >2 & any(Pmat < -smallpos)) {
                cat ("stop1\n"); browser()}
            if (any(Pmat > (1+smallpos) | Pmat < -smallpos)) return("underflow")
