...
# Fails for msm 3/2025, due to my non-handling of obstrue (I think)
...
# Now do derivatives.  There are two issues.
#  first is that the derivatives are on the internal scale, and change
# when the data is scaled.
#  second is that hmm treats the initial parameter for the intercept
# as the intercept at the mean X value.  The solution is to pre-center
# our sex variable, and use scale=FALSE.

msex <- cav$sex -  attr(mfit4a$data$mm.cov, "means") # their center
hfit4a <- hmm(cbind(years, state) ~ msex, data=cav, mc.cores=5,
...
for (i in 1:7) {
    ...
    tfit <- hmm(cbind(years, state) ~ msex, data=cav, mc.cores=10,
                id = PTNUM, qmatrix=Qm, rfun=errfun, pfun=cinit,
                death=4, otype=otype,  rcoef=rcoef, qcoef= qtemp, 
                mfun= "hmmtest", scale=FALSE) 
    dvec4[i] <- logLik(tfit)
}
for (i in 1:4) {
    rtemp <- rcoef
    ...
    tfit <- hmm(cbind(years, state) ~ msex, data=cav, mc.cores=10,
                ...
                mfun= "hmmtest", scale=FALSE)
    dvec4[i+7] <- logLik(tfit)
}    
d4 <- (dvec4 - logLik(hfit4a))/eps
...
all.equal(d4, mfit4a$deriv[-(8:10)]/-2, tolerance=sqrt(eps))

# check the hmmgrad function
#  the derivative ends up labeled as "loglik" (no user would ever do this)
hfit4c <- hmm(cbind(years, state) ~ msex, data=cav, mc.cores=5,
              id = PTNUM, qmatrix=Qm, rfun=errfun, pfun=cinit,
              death=4, otype=otype,  rcoef=rcoef, qcoef= q4, 
              mfun= "hmmtest", mpar=list(fn="hmmgrad"),
              scale= FALSE) 
aeq(hfit4a$fit$deriv, hfit4c$loglik[-1])

#Once we iterate, then a difference in initial values doesn't matter
...
hfit4f <- hmm(cbind(years, state) ~ sex, data=cav, mc.cores=6,
            id = PTNUM, qmatrix=Qm, rfun=errfun, pfun=cinit,
            death=4, otype=otype,  rcoef=rcoef, qcoef= q4, 
            mfun=hmmscore, mpar=list(gr="hmmboth", eps=1e-7))

aeq(-2*hfit4$loglik[2], mfit4$minus2loglik)
all.equal( hfit4$loglik, hfit4f$loglik, tol=1e-7)
...
aeq(hfit4$coefficients, mfit4$estimates[indx], tol=1e-2)
...
hfit4x <- hmm(cbind(years, state) ~ sex, data=cav, mc.cores=6,
              ...
              mfun="hmmtest", mpar=list(fn="hmmboth"), 
              icoef = hfit4$coef)
all.equal(hfit4x$loglik[2], hfit4$loglik[2])

# Total iterations is surprising, given that we start at the same place.
